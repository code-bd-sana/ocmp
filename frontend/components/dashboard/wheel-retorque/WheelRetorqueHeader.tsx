...
interface WheelRetorqueHeaderProps {
  clientName?: string;
  searchQuery: string;
  onSearchChange: (value: string) => void;
}

export default function WheelRetorqueHeader({
  clientName,
  searchQuery,
  onSearchChange,
}: WheelRetorqueHeaderProps) {
  return (
    <div className="">
      <div className="flex flex-col justify-between gap-y-3 md:flex-row md:gap-x-3">
        <h1 className="text-primary mb-2 text-3xl font-bold">
          {clientName ? `${clientName} — Wheel Torque` : "Wheel Torque"}
        </h1>
        <div className="relative flex max-w-xl items-center text-(--input-foreground)">
          ...
          <Input
            type="text"
            placeholder="Search by wheel torque details"
            value={searchQuery}
