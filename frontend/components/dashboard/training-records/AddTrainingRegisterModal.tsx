...
import { CreateTrainingRegisterInput } from "@/lib/training-register/training-register.types";
...
type AddRegisterForm = z.infer<typeof addRegisterSchema>;

interface AddTrainingRegisterModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  standAloneId?: string;
  onSubmit: (data: CreateTrainingRegisterInput) => Promise<void> | void;
}
...
export default function AddTrainingRegisterModal({
  open,
  onOpenChange,
  standAloneId,
  onSubmit,
}: AddTrainingRegisterModalProps) {
  const {
    register,
    handleSubmit,
    watch,
  ...
  useEffect(() => {
    ...
    const loadIntervals = async () => {
      try {
        ...
        if (nextIntervals.length > 0) {
          setValue("trainingInterval", String(nextIntervals[0]));
        } else {
          setValue("trainingInterval", "");
        }
      } catch (err) {
    ...
    loadIntervals();
  }, [selectedTrainingId, setValue, standAloneId]);
  ...
  const submit = async (values: AddRegisterForm) => {
    await onSubmit({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-150">
        <DialogTitle>Add Training Register</DialogTitle>
        ...
        >
          <div className="space-y-2">
            ...
            <select
              ...
              onChange={(e) =>
                setValue("participantId", e.target.value, {
                  shouldValidate: true,
                })
              }
