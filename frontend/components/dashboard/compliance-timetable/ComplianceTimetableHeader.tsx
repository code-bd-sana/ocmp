...
interface ComplianceTimetableHeaderProps {
  clientName?: string;
  searchQuery: string;
  onSearchChange: (value: string) => void;
}

export default function ComplianceTimetableHeader({
  clientName,
  searchQuery,
  onSearchChange,
}: ComplianceTimetableHeaderProps) {
  return (
    <div>
      <div className="flex flex-col justify-between gap-y-3 md:flex-row md:gap-x-3">
        <h1 className="text-primary mb-2 text-3xl font-bold">
          {clientName
            ? `${clientName} - Compliance Timetable`
        ...
        <div className="relative flex max-w-xl items-center text-(--input-foreground)">
          ...
          <Input
            type="text"
            placeholder="Search by task or responsible party"
            value={searchQuery}
