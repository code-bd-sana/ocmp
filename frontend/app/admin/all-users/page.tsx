...
const FILTER_TO_ROLE: Record<string, UserListRoleFilter> = {
  All: "all",
...
const columns: Column<AllUsersRow>[] = [
  {
    key: "name",
    title: "NAME",
    render: (row) => (
...
export default function AdminAllUsersPage() {
  const [rows, setRows] = useState<AllUsersRow[]>([]);
  ...
  const fetchUsers = useCallback(async () => {
    try {
      ...
      const role = FILTER_TO_ROLE[activeRoleFilter] ?? "all";
  ...
  }, [activeRoleFilter, currentPage, debouncedSearchValue]);
        ...
        endActionGroup: [
          ...
          },
          {
            ...
            onClick: (value) => {
              ...
              setActiveRoleFilter(nextFilter);
              setCurrentPage(1);
            },
            filter: true,
            options: ["All", "Transport Manager", "Client"],
            visibility: true,
      ...
      <Card className="rounded-sm border-none bg-[#f8f9fc] shadow-[0_6px_18px_rgba(13,75,159,0.08)]">
        <CardContent className="p-3 sm:p-4 md:p-5">
          <div className="mb-3 flex flex-col items-start gap-1 text-base text-[#8F8F8F] sm:flex-row sm:items-center sm:justify-between md:text-xl">
            <span>Total Users: {totalData}</span>
            {isLoading ? (
              <span
