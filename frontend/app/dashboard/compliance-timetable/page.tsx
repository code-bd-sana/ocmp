...
export default function ComplianceTimetableRedirectPage() {
    ...
    const redirectBasedOnRole = async () => {
      try {
        ...
        if (userRole === "STANDALONE_USER") {
          ...
          if (userId) {
            router.replace(`/dashboard/compliance-timetable/${userId}`);
          } else {
          ...
          if (clientRes.status && clientRes.data?.data?.length) {
            ...
            router.replace(`/dashboard/compliance-timetable/${firstClientId}`);
          } else {
  ...
  return (
    <div className="container mx-auto max-w-6xl py-10">
      <div className="flex h-64 items-center justify-center">
        <p className="text-muted-foreground">Loading compliance timetable...</p>
      </div>
    </div>
  );
}
