...
/**
 * Landing page for /dashboard/driver-tachograph (no standAloneId).
 * Fetches the client list and redirects to the first client's driver tachograph list
 * so that the sidebar auto-selects the first client.
 */
export default function DriverTachographRedirectPage() {
    ...
    const redirectBasedOnRole = async () => {
      try {
        ...
        if (userRole === "STANDALONE_USER") {
          ...
          if (userId) {
            router.replace(`/dashboard/driver-tachograph/${userId}`);
          } else {
          ...
          if (clientRes.status && clientRes.data?.data?.length) {
            ...
            router.replace(`/dashboard/driver-tachograph/${firstClientId}`);
          } else {
  ...
  return (
    <div className="container mx-auto max-w-6xl py-10">
      <div className="flex h-64 items-center justify-center">
        <p className="text-muted-foreground">Loading driver tachographs...</p>
      </div>
    </div>
  );
}
