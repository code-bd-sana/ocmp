...
/**
 * Landing page for /dashboard/vehicle-list (no standAloneId).
 *
 * For STANDALONE_USER: Uses their own user ID as standAloneId
 * For TRANSPORT_MANAGER: Fetches the client list and redirects to the first client's vehicle list
 */
export default function VehicleListRedirectPage() {
    ...
    const redirectBasedOnRole = async () => {
      try {
        ...
        if (userRole === "STANDALONE_USER") {
          ...
          if (userId) {
            router.replace(`/dashboard/vehicle-list/${userId}`);
          } else {
          ...
          if (clientRes.status && clientRes.data?.data?.length) {
            ...
            router.replace(`/dashboard/vehicle-list/${firstClientId}`);
          } else {
