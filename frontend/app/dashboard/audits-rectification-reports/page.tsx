...
export default function AuditRectificationRedirectPage() {
    ...
    const redirectBasedOnRole = async () => {
      try {
        ...
        if (userRole === "STANDALONE_USER") {
          if (userId) {
            router.replace(`/dashboard/audits-rectification-reports/${userId}`);
          } else {
        ...
        if (res.status && res.data?.data?.length) {
          ...
          router.replace(
            `/dashboard/audits-rectification-reports/${firstClientId}`,
          );
        } else {
