...
import {
  ...
  SearchRenewalTrackerQueryInput,
...
export const createRenewalTrackerAsManager = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const body = req.body as CreateRenewalTrackerAsManagerInput;
  const createdBy = new mongoose.Types.ObjectId(req.user!._id);
  const result = await renewalTrackerServices.createRenewalTrackerAsManager({ ...body, createdBy });
...
export const createRenewalTrackerAsStandAlone = catchAsync(async (req: AuthenticatedRequest, res: Response) => {
  const body = req.body as CreateRenewalTrackerAsStandAloneInput;
  const createdBy = new mongoose.Types.ObjectId(req.user!._id);
  const result = await renewalTrackerServices.createRenewalTrackerAsStandAlone({ ...body, createdBy });
  ServerResponse(res, true, 201, 'Renewal tracker created successfully', result);
});
...
export const getManyRenewalTrackerAsManager = catchAsync(
  async (req: AuthenticatedRequest, res: Response) => {
    const query = {
      ...((req as any).validatedQuery as SearchRenewalTrackerQueryInput),
