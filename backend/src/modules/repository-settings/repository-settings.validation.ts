...
const zodUpdateRepositorySettingsSchema = z
  .object({
    ...
    driverDetailsLicenceAndDoc: z
      .boolean({ message: 'driverDetailsLicenceAndDoc must be a boolean' })
      .optional(),
    driverTachoGraphAndWTDInfringements: z
      .boolean({ message: 'driverTachoGraphAndWTDInfringements must be a boolean' })
      .optional(),
    trainingAndToolboxTalks: z
      .boolean({ message: 'trainingAndToolboxTalks must be a boolean' })
      .optional(),
    renewalsTracker: z.boolean({ message: 'renewalsTracker must be a boolean' }).optional(),
    OCRSChecksAndRectification: z
      .boolean({ message: 'OCRSChecksAndRectification must be a boolean' })
      .optional(),
    trafficCommissionerCommunicate: z
      .boolean({ message: 'trafficCommissionerCommunicate must be a boolean' })
      .optional(),
    ...
    PG9sPG13FGClearanceInvesting: z
      .boolean({ message: 'PG9sPG13FGClearanceInvesting must be a boolean' })
      .optional(),
    contactLog: z.boolean({ message: 'contactLog must be a boolean' }).optional(),
    GV79DAndMaintenanceProvider: z
      .boolean({ message: 'GV79DAndMaintenanceProvider must be a boolean' })
      .optional(),
    complianceTimetable: z.boolean({ message: 'complianceTimetable must be a boolean' }).optional(),
    auditsAndRectificationReports: z
      .boolean({ message: 'auditsAndRectificationReports must be a boolean' })
      .optional(),
    fuelUsage: z.boolean({ message: 'fuelUsage must be a boolean' }).optional(),
    wheelRetorquePolicyAndMonitoring: z
      .boolean({ message: 'wheelRetorquePolicyAndMonitoring must be a boolean' })
      .optional(),
    workingTimeDirective: z
      .boolean({ message: 'workingTimeDirective must be a boolean' })
      .optional(),
    policyProcedureReviewTracker: z
      .boolean({ message: 'policyProcedureReviewTracker must be a boolean' })
      .optional(),
    subcontractorDetails: z
      .boolean({ message: 'subcontractorDetails must be a boolean' })
      .optional(),
    trainingRecords: z.boolean({ message: 'trainingRecords must be a boolean' }).optional(),
  })
  .strict()
